...
void Deeplayout::SaveTextData(const QString & filename)
{
	...
	stream << "FrontDoor" << Qt::endl;
	stream << "{" << Qt::endl;
	int dir = house.front_door.entry.dir;
	QRect rect = house.front_door.entry.rect;
	stream << rect.x() << " " << rect.y() << " " << rect.width() << " " << rect.height() << " " << ORIENTED[dir].dir_string << Qt::endl;
	stream << "}" << "\n";
	stream << Qt::endl;

	stream << "ExteriorWall" << Qt::endl;
	stream << "{" << Qt::endl;
	for (int i = 0; i < house.exterior_wall.size(); i++)
	{
		DIRECTEDPOINT point = house.exterior_wall[i];
		stream << point.point.rx() << " " << point.point.ry() << Qt::endl;
	}
	stream << "}" << Qt::endl;
	stream << Qt::endl;

	stream << "InteriorWall" << Qt::endl;
	stream << "{" << Qt::endl;
	...
	for (int i = 0; i < house.rooms.size(); i++)
	{
		...
		stream << room.label.name << Qt::endl;
		stream << "{" << Qt::endl;

		stream << "Boundary" << Qt::endl;
		stream << "{" << Qt::endl;
...
{
    qDebug() << "SaveImageData called with textState:" << textState;
    qDebug() << "Number of rooms:" << house.rooms.size();
    ...
    for (int i = 0; i < house.rooms.size(); i++) {
        ...
        // Add debug output for text rendering
        if (textState) {
            qDebug() << "Drawing text for room:" << house.rooms[i].label.name;
            qDebug() << "Room boundaries:" << house.rooms[i].boundary.size() << "points";
		...
		else
		{
			...
			else
			{
				...
				for (int j = 0; j < contactWalls.size(); j++)
				{
					...
					{
						if (rooms[i].label.name == "Storage" || rooms[i].label.name == "SecondRoom" || rooms[i].label.name == "MasterRoom")
						{
...
void Deeplayout::HouseAbstract()
{
    ...
    // Debug exterior walls
    qDebug() << "\nExterior Wall Points:";
    for (int i = 0; i < house.exterior_wall.size(); i++) {
        qDebug() << QString("Point %1: (%2, %3)")
            .arg(i)
            .arg(house.exterior_wall[i].point.x())
            .arg(house.exterior_wall[i].point.y());
    }
    
    // Debug interior walls
    qDebug() << "\nInterior Walls:";
    for (int i = 0; i < house.interior_wall.size(); i++) {
        qDebug() << QString("\nWall %1 points:").arg(i);
        for (int j = 0; j < house.interior_wall[i].size(); j++) {
            qDebug() << QString("  Point %1: (%2, %3)")
                .arg(j)
                .arg(house.interior_wall[i][j].point.x())
                .arg(house.interior_wall[i][j].point.y());
        }
    }
    
    // Debug rooms
    qDebug() << "\nRooms:";
    for (const ROOM& room : house.rooms) {
        qDebug() << QString("\nRoom: %1").arg(room.label.name);
        qDebug() << "  Boundary points:";
        for (int i = 0; i < room.boundary.size(); i++) {
            qDebug() << QString("    Point %1: (%2, %3)")
                .arg(i)
                .arg(room.boundary[i].point.x())
                .arg(room.boundary[i].point.y());
        }
        qDebug() << QString("  Entry door: (x=%1, y=%2, w=%3, h=%4)")
            .arg(room.entry.entry.rect.x())
            .arg(room.entry.entry.rect.y())
            .arg(room.entry.entry.rect.width())
            .arg(room.entry.entry.rect.height());
        
        qDebug() << "  Windows:";
        for (int i = 0; i < room.windows.size(); i++) {
            qDebug() << QString("    Window %1: (x=%2, y=%3, w=%4, h=%5)")
                .arg(i)
                .arg(room.windows[i].rect.x())
                .arg(room.windows[i].rect.y())
                .arg(room.windows[i].rect.width())
                .arg(room.windows[i].rect.height());
        }
    }
    
    // Debug front door
    qDebug() << QString("\nFront Door: (x=%1, y=%2, w=%3, h=%4)")
        .arg(house.front_door.entry.rect.x())
        .arg(house.front_door.entry.rect.y())
        .arg(house.front_door.entry.rect.width())
        .arg(house.front_door.entry.rect.height());
