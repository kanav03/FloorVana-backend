...
#     handlers=[
#         logging.FileHandler("synth_api.log"),
...
# def analyze_input_image(input_path):
#     """Analyze input image and log its properties for debugging"""
#     try:
...
#             shape_info = f"Array shape: {img_array.shape}, dtype: {img_array.dtype}"
            
#             # Get channel info if available
#             channel_info = ""
#             if len(img_array.shape) == 3 and img_array.shape[2] >= 3:
#                 r_unique = len(np.unique(img_array[:,:,0]))
#                 g_unique = len(np.unique(img_array[:,:,1]))
#                 b_unique = len(np.unique(img_array[:,:,2]))
#                 channel_info = f"Unique values count per channel: R:{r_unique}, G:{g_unique}, B:{b_unique}"
                
#                 if img_array.shape[2] > 3:  # Has alpha channel
#                     a_unique = len(np.unique(img_array[:,:,3]))
#                     channel_info += f", A:{a_unique}"
            
#             logger.info(f"{shape_info}, {channel_info}")
#             return True
#     except Exception as e:
#         logger.error(f"Error analyzing input image: {e}")
#         return False
...
#         input_path = os.path.join(UPLOAD_FOLDER, filename)
#         file.save(input_path)
#         logger.info(f"Saved file at {input_path}")
        
#         # Analyze the input image for debugging
#         if not analyze_input_image(input_path):
#             return jsonify({'error': 'Failed to analyze input image'}), 400
...
#             # This can throw exceptions with iOS images
#             synthesizer.floorplan = LoadFloorplanSynth(input_path)
#             synthesizer.map = synthesizer.floorplan.input_map
#             synthesizer.map.flags.writeable = True
...
#             # Save synthesis result
#             synth_output_path = synthesizer.save_synth_result(OUTPUT_FOLDER, filename)
#             logger.info(f"Synthesis complete. Output saved at: {synth_output_path}")
#         except Exception as synthesis_error:
#             logger.error(f"Synthesis failed: {synthesis_error}")
#             logger.error(traceback.format_exc())
#             return jsonify({
#                 'error': f'Synthesis processing failed: {str(synthesis_error)}',
#                 'details': 'The image format may not be compatible with our system. Please use a PNG or JPG image with proper boundary markings.'
#             }), 400

#         # Wait for synthesis to complete and files to be ready
#         logger.info(f"Waiting {SYNTHESIS_WAIT_TIME} seconds for synthesis to stabilize...")
#         time.sleep(SYNTHESIS_WAIT_TIME)
...
#         for floorplan_name in floorplans:
#             logger.info(f"Normalizing: {OUTPUT_FOLDER}/{floorplan_name}")
#             input_floorplan_path = os.path.join(OUTPUT_FOLDER, floorplan_name)
#             output_floorplan_path = os.path.join(NORMALIZED_FOLDER, floorplan_name)
...
#             logger.debug(f"Input map shape: {input_map.shape}")
#             abstracter = WallAbstract(input_map) 
#             abstracter.exterior_boundary_abstract()
#             abstracter.interior_wall_abstract() 
...
#             output = Image.fromarray(np.uint8(output_map))
#             output.save(output_floorplan_path)
#             logger.info(f"Normalization complete for: {floorplan_name}")
...
#             logger.info(f"Visualization complete. Output saved at: {visualization_output}")
