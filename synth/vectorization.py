        ...
        for h in range(data_size):  
            for w in range(data_size):
                if self.boundary_map[h, w] > 0:
                    self.house_min_h = min(self.house_min_h, h)
                    self.house_max_h = max(self.house_max_h, h)
                    self.house_min_w = min(self.house_min_w, w)
                    self.house_max_w = max(self.house_max_w, w)
        self.house_min_h = max(self.house_min_h - 10, 0)
        self.house_max_h = min(self.house_max_h + 10, data_size - 1)
        self.house_min_w = max(self.house_min_w - 10, 0)
        self.house_max_w = min(self.house_max_w + 10, data_size - 1)
            ...
            for w in range(self.house_min_w, self.house_max_w):
                if not in_line:
                    ...
                    if wall_map[location, w] != 127:
                        ...
                        in_line = False
            if in_line:
                w_max = self.house_max_w - 1
                lines.append((w_min, w_max))
        else:  # Vertical
            in_line = False
            for h in range(self.house_min_h, self.house_max_h):
                if not in_line:
                    if wall_map[h, location] == 127:
                        h_min = h
                        in_line = True
                else:
                    if wall_map[h, location] != 127:
